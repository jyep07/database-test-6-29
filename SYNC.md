...
```json
{
  ...
  "docs": {
    "<docs/path/to/file.md>": { "noteId": "<slite note id>", "repoHash": "<sha256>", "sliteHash": "<sha256>" }
  }
}
```
...
2. **Leave a comment** on one Slite note (anchor a comment to a value on any mapped
   note and write the correction). Optionally also edit a repo file to exercise the
   git→Slite direction.
...
- Run a supervised first cycle on your repo (see "Testing it / supervised first run"):
  a Routine A dry run to produce a sync PR, then the full Routine B apply + resolve on a
  fresh comment.
